...
-- What gets stored in a session file (used by auto-session).
-- `localoptions` is required by auto-session; `terminal` is deliberately
...
require('lazy').setup({
  ...
  -- NOTE: Fuzzy finding is handled by `snacks.picker`, configured in
  -- `lua/custom/plugins/snacks.lua`. It replaced telescope.nvim, which drives
  -- its preview highlighting through nvim-treesitter v1's Lua API and so
  -- errors on every preview against the v2 (`main`) branch used here.
  -- The `<leader>s*` keymaps below are unchanged from the telescope setup.
  ...
  { -- Highlight, edit, and navigate code (nvim-treesitter v2 on `main`; see `:h nvim-treesitter`)
    'nvim-treesitter/nvim-treesitter',
    branch = 'main',
    lazy = false,
    build = ':TSUpdate',
    config = function()
      if not vim.version.ge(vim.version(), { major = 0, minor = 12, patch = 0 }) then
        vim.notify('nvim-treesitter v2 requires Neovim 0.12+. Upgrade Neovim, then restart.', vim.log.levels.ERROR)
        return
      end
      ...
      vim.api.nvim_create_autocmd('FileType', {
        desc = 'Treesitter highlight, fold, indent (nvim-treesitter v2)',
